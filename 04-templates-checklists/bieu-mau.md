---
layout:
  width: wide
  title:
    visible: true
  description:
    visible: true
  tableOfContents:
    visible: true
  outline:
    visible: false
  pagination:
    visible: true
  metadata:
    visible: false
  tags:
    visible: true
  actions:
    visible: true
---
# 📋 Biểu mẫu
Các biểu mẫu sử dụng xuyên suốt quy trình tổ chức kỳ thi ÖSD.
{% content-ref url="ke-hoach-ky-thi.md" %}
[ke-hoach-ky-thi.md](ke-hoach-ky-thi.md)
{% endcontent-ref %}
Mẫu kế hoạch tổ chức kỳ thi trình Exam Director phê duyệt — Bước 1.
{% content-ref url="danh-sach-thi-cac-ky-nang.md" %}
[danh-sach-thi-cac-ky-nang.md](danh-sach-thi-cac-ky-nang.md)
{% endcontent-ref %}
Mẫu danh sách phân bổ thí sinh theo kỹ năng, phòng, giờ thi — Bước 3.
{% content-ref url="to-phan-loai-de-thi.md" %}
[to-phan-loai-de-thi.md](to-phan-loai-de-thi.md)
{% endcontent-ref %}
Mẫu tờ phân loại đề thi theo phòng/buổi thi — Bước 6.
{% content-ref url="danh-sach-check-in.md" %}
[danh-sach-check-in.md](danh-sach-check-in.md)
{% endcontent-ref %}
Mẫu danh sách điểm danh thí sinh tại phòng thi — Bước 7.
{% content-ref url="danh-sach-giu-do.md" %}
[danh-sach-giu-do.md](danh-sach-giu-do.md)
{% endcontent-ref %}
Mẫu danh sách gửi giữ đồ cá nhân của thí sinh — Bước 7.
{% content-ref url="protokol.md" %}
[protokol.md](protokol.md)
{% endcontent-ref %}
Mẫu biên bản Protokol ghi nhận diễn biến buổi thi — Bước 7, 9.
